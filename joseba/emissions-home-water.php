...
if(!isset($_SESSION['id_user'])) {
	...
	$emissions_water_emissions_per_year = "0.00";
	$emissions_array = $database->get_results( "SELECT * FROM ".TABLE_DB_USER_EMISSIONS_HOME_WATER." WHERE id_user=" . $id_user . " " );
}
?>
	...
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		data: [              
		{
			...
			foreach( $emissions_array as $emission ) {
				...
				$emissions_water_emissions_per_year = $emission['water_emissions_per_year'];
				echo "{ label: \"".$emissions_date."\",  y: ".$emissions_water_emissions_per_year."}";
...
<p style="text-align:center; padding:1em; border:1px solid orange; color:orange;">Your water consumption produces <span id="emissions_home_water_tones"><?php echo $emissions_water_emissions_per_year; ?></span> tons of carbon a year</p>
...
<?php
	foreach( $emissions_array as $emission ) {
		...
		if ($emissions_units==1) {
			$emissions_units = "gals";
		} else {
			$emissions_units = "tgals";
		}
		$emissions_lowest_use = $emission['lowest_use'];
		$emissions_highest_use = $emission['highest_use'];
		$emissions_water_use_per_year = $emission['water_use_per_year'];
		$emissions_water_emissions_per_year = $emission['water_emissions_per_year'];
		...
		echo "<td>".$emissions_water_use_per_year."</td>";
		echo "<td>".$emissions_units."</td>";
		echo "<td>".$emissions_water_emissions_per_year."</td>";
		echo "<td><a href=\"".$url_emissions_home_water_delete."?emissions_id=".$emissions_id."\">Delete</a></td>";
		echo "</tr>";
	}
?>
</table>
